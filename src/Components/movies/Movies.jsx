...
const Movies = ({categories, index, page, setPage}) => {
    ...
    useEffect(()=>{
        const getMovies = async () => {
            try{
                const result = await axios.get(`${Base_Endpoint}/movie/${categories[index].toLowerCase().replace(/\s+/g, '_')}?api_key=${api_key}&page=${page}`);
    ...
    return (
        ...
        >
            {loading ? 
                ...
                filtered && 
                <>
                    <GenreFilter movies={movies} setFiltered={setFiltered} currentGenre={currentGenre} setCurrentGenre={setCurrentGenre} />
